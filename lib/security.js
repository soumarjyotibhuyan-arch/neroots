...
export function isAuthorizedAdminEmail(email, db = null) {
  if (!email || typeof email !== 'string') return false;
  const cleanEmail = email.trim().toLowerCase();

  // 1. Core store owners
  if (cleanEmail === 'soumarjyotibhuyan@gmail.com' || cleanEmail === 'utpalabhuyan29@gmail.com') {
    return true;
  }

  // 2. Check process.env.ADMIN_EMAILS & NEXT_PUBLIC_ADMIN_EMAILS
  const envEmails = [
  ...
  // 3. Check db.adminUsers if provided
  if (db && Array.isArray(db.adminUsers)) {
    if (db.adminUsers.some(u => u.email && u.email.trim().toLowerCase() === cleanEmail)) {
      return true;
    }
  }

  // 4. Check global in-memory whitelist
  if (typeof global !== 'undefined' && global._neroots_admin_whitelist && global._neroots_admin_whitelist.has(cleanEmail)) {
    return true;
  }
...
export function validateAdminRequest(req, res) {
  ...
  // If token is a valid format (e.g. from recent restart or cross-lambda), accept if matches prefix
  if (token.startsWith('gauth_') || token.startsWith('gsec_') || token.startsWith('auth_') || token.startsWith('admin_')) {
